...
import { getSum } from "./core/getSum.js";
...
router.all("/", (_request, response, next) => {
  try {
    return response
      .status(200)
      .json({ app: "AWS Lambda NodeJS Express JS App", version: "1.0.0" });
  } catch (error) {
    next(error);
  }
});
...
router.get("/hello", async (_request, response, next) => {
  try {
    return response
      .status(200)
      .json({ app: "Hello from path", version: "1.0.0" });
  } catch (error) {
    next(error);
  }
});
...
router.get("/sum", async (request, response, next) => {
