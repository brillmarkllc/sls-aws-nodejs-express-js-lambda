...
This boilerplate project sets up a Serverless Framework application with AWS Lambda and Node.js runtime, allowing you to deploy an Express.js-based API in a serverless environment using Javascript. This setup is highly modular, with functions packaged individually and enhanced with plugins for local development.
...
```
Deploying "sls-aws-nodejs-express-js-lambda" to stage "dev" (us-east-1)

✔ Service deployed to stack sls-aws-nodejs-express-js-lambda-dev (96s)

endpoint: ANY - https://xxxxxxxxxx.execute-api.us-east-1.amazonaws.com
functions:
  api: sls-aws-nodejs-express-js-lambda-dev-api (2.3 kB)
```
...
```json
{
  "app": "AWS Lambda NodeJS Express JS App",
  "version": "1.0.0"
}
```
...
- **Express App Integration**: The Express app, located in `src/app.js`, is imported and passed to `serverless`, which handles the necessary adaptation to run in a Lambda environment.
...
- **Routing**:
  ...
  - Routes are imported from `routes.js` and attached to the app to handle API requests for various endpoints.
...
- **Sum Route (`/sum`)**:
  ...
  - This route calls the `getSum` function from the `core/getSum.js` module. It handles a GET request and uses a core function to return a calculated sum, showing how core logic can be abstracted from the route handler.
...
- **Core Sum Calculation**:
  ...
  - The `getSum` function abstracts the logic for summing two numbers. In this example, it calculates the sum of `2` and `5` using the `sum()` utility function from `utils/math.js`.
...
- **Success Response**:
  ...
  - On a successful calculation, the result is wrapped in an object and sent back using the `successResponse()` function from `response.js`. The response could also be the raw sum if preferred.
